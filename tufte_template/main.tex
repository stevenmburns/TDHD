...
\end{scalacode}
\vspace{-12\baselineskip}\footnotetext{%
Line 5: we return a silly number first to ensure the test fails
}%
...
We know that the test failed and can now change the implementation so that it work for this test.
\begin{scalacode}
// src/main/scala/tdhd.scala
package tdhd
object PopCount {
...
\vspace{7\baselineskip}\footnotetext{%
You can do this functionally if you would rather.
Something like {\tt (0 until 8) map {i => (n & (BigInt(1)<<i)) != BigInt(0)} sum}
}\vspace{-7\baselineskip}%
\begin{scalacode}
// src/main/scala/tdhd.scala
package tdhd
